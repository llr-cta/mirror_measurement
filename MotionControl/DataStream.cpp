...
StringDataStream::StringDataStream(std::string response, int loud):
  DataStream(loud), m_ostream(), m_istring(response)
{
  // nothing to see here
}

StringDataStream::~StringDataStream()
{
  // nothing to see here
}

void StringDataStream::
sendData(const datastring& out, long to_sec, long to_usec)
{
  m_ostream << out;
}

datastring StringDataStream::
recvData(size_t req_count, long to_sec, long to_usec)
{
  size_t icount = m_istring.size();
  if(icount>req_count)
    {
      datastring ds = m_istring.substr(icount);
      m_istring = m_istring.substr(0,icount);
      return ds;
    }
  else if(icount)
    {
      datastring ds = m_istring;
      m_istring = std::string();
      return ds;
    }
  else
    {
...
void StringDataStream::resetDataStream()
{
  // nothing to see here
}

std::string StringDataStream::udsl() const
{
  return std::string("ostring:none");
}
...
void UDPDataStream::resetDataStream()
{
  ...
  if(loud()>=2)
    Debug::stream() << "UDPDataStream::resetDataStream()" << std::endl;
  ...
  do
      ...
      if(n>0)
	{
	  ...
	  if(loud()>=2)
	    {
	      Debug::stream() << "UDPDataStream::resetDataStream(): ";
